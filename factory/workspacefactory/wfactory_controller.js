...
WorkspaceFactoryController.prototype.addCategory = function() {
  ...
  // Get name from user.
  var name = this.promptForNewCategoryName('新規カテゴリ名を入力して下さい：');
...
console.log(this.model.getSelected().type == 'category');
  // Check if user wants to remove current category.
  var check = confirm('選択している'
        + ((this.model.getSelected().type == 'category') ? 'カテゴリ' : 'セパレータ') + 'を削除してもよろしいですか？');
  // this.model.getSelected().type == 'category'だとfalse
  // var check = confirm('選択している'
  //       + this.model.getSelected().type == 'category' ? 'カテゴリ' : 'セパレータ' + 'を削除してもよろしいですか？');
  ...
  if (!nextId) {
    alert('カテゴリやセパレータが1つもありません。全てのブロックはカテゴリ分けされずに表示されます。');
...
WorkspaceFactoryController.prototype.exportXmlFile = function(exportMode) {
  ...
  if (exportMode == WorkspaceFactoryController.MODE_TOOLBOX) {
    var fileName = prompt('ツールボックスのファイル名：', 'toolbox.xml');
  } else {
    var fileName = prompt('ワークスペースに初期配置するブロックのファイル名：',
                          'workspace.xml');
  }
...
WorkspaceFactoryController.prototype.exportInjectFile = function() {
  var fileName = prompt('ワークスペースの定義のファイル名：',
                        'workspace.js');
  if (!fileName) {  // If cancelled.
    return;
  }
...
WorkspaceFactoryController.prototype.changeCategoryName = function() {
  ...
  window.foo = selected;
  var newName = this.promptForNewCategoryName('新しいカテゴリ名を入力して下さい。', selected.name);
    ...
    try {
      ...
      if (importMode == WorkspaceFactoryController.MODE_TOOLBOX) {
        ...
        if (hasToolboxElements &&
            !confirm('現在のツールボックスは失われますがよろしいですか？')) {
            return;
        }
        // Import toolbox XML.
        ...
        if (controller.toolboxWorkspace.getAllBlocks().length > 0 &&
            !confirm('現在のワークスペースのブロックは失われますがよろしいですか？')) {
            return;
        }
...
WorkspaceFactoryController.prototype.clearAll = function() {
  if (!confirm('編集中のワークスペースをリセットしてもよろしいですか？')) {
    return;
  }
  ...
  reader.onload = function() {
    try {
      ...
      var blockTypes = FactoryUtils.defineAndGetBlockTypes(reader.result,
          ...
          console.log('cmh',controller.model.hasDefinedBlockTypes(blockTypes));
      ...
      if (controller.model.hasDefinedBlockTypes(blockTypes) &&
          !confirm('読み込むブロックに、既にツールボックスにあるブロックと同じ名前が存在します。現在定義されているブロックを上書きしてもよろしいですか？')) {
          return;
      }
      ...
      alert('ブロックの定義が記述されたファイルを選択して下さい。');
