...
WorkspaceFactoryView.prototype.addCategoryRow = function(name, id) {
  ...
  if (count == 0) {
    document.getElementById('categoryHeader').textContent = 'カテゴリ一覧';
  }
...
WorkspaceFactoryView.prototype.updateHelpText = function(mode) {
  if (mode == WorkspaceFactoryController.MODE_TOOLBOX) {
    var helpText = 'ツールボックスに設定するブロックをワークスペースへドラッグして下さい。';
  } else {
    var helpText = 'カスタムワークスペースに初期配置するブロックをワークスペースへドラッグして下さい。'
  }
